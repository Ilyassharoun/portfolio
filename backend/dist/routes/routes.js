...
const project_reviews_1 = __importDefault(require("../models/project_reviews"));
...
router.use('/assets', express_1.default.static(path_1.default.join(__dirname, '../assets')));
// CV download route
/**
 * @swagger
 * /api/cv:
...
router.get('/cv', (req, res) => {
    const filePath = path_1.default.join(__dirname, '../../../backend/assets/HarounIlyass.pdf');
...
router.get('/routes/projects', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../assets/projects.html'));
});
// Track visited route
...
router.get('/projects', async (req, res) => {
    try {
        const projects = await Projects_1.default.find().sort({ createdAt: -1 });
 ...
 *         description: Server error
 */
router.get('/projects/projectid/:projectId', async (req, res) => {
    try {
        const project = await Projects_1.default.findOne({ projectId: req.params.projectId });
        if (!project) {
            return res.status(404).json({ message: 'Project not found' });
        }
        res.status(200).json(project);
...
router.get('/projects/category/:category', async (req, res) => {
    try {
        const projects = await Projects_1.default.find({ category: req.params.category });
        res.status(200).json(projects);
    }
    catch (error) {
...
router.post('/review', async (req, res) => {
    try {
        ...
        // Create new review
        const newReview = new project_reviews_1.default({
            projectId,
            name,
            rating,
            comment,
...
router.get('/reviews/:projectId', async (req, res) => {
    try {
        ...
        const reviews = await project_reviews_1.default.find({ projectId }).sort({ createdAt: -1 });
